...
def parse_smartcat_report(
    ...
    for worksheet in root.findall(f"{{{SMARTCAT_NS}}}Worksheet"):
        ...
        if statistics_rows:
            logger.debug("Statistics found in worksheet '%s'", sheet_name)
            break
...
def _parse_smartcat_task_statistics(
    ...
    if analyse_elements:
        logger.debug(
            "Found %d analyse elements for fallback parsing", len(analyse_elements)
        )
        for idx, analyse in enumerate(analyse_elements, 1):
            ...
            processed_any = True
            logger.debug("Analyse block #%d", idx)
            for name in ROW_NAMES:
                amount = analyse_values[name]
                if amount > 0:
                    values[name] += amount
