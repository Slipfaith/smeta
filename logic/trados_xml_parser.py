...
def _extract_language_from_taskinfo(taskinfo: ET.Element) -> str:
    ...
    if lang_element is not None:
        ...
        logger.debug(
            "  Language element found: name='%s', lcid='%s'", lang_name, lcid
        )
...
def _parse_trados_report(
    ...
    try:
        ...
        if src_lang and tgt_lang:
            ...
            logger.info(
                "Language pair from filename: %s → %s", src_lang, tgt_lang
            )
            if (
                taskinfo_lang
                and len(tgt_lang) <= 3
        ...
        logger.debug("Found %s file elements", len(file_elements))
...
def parse_reports(
    ...
    logger.info("Processing finished: %s/%s reports processed", successfully_processed, len(paths))
    ...
    for i, (pair_key, values) in enumerate(sorted_pairs, 1):
        ...
        for name, value in values.items():
            if value > 0:
                logger.debug("    %s: %s", name, f"{value:,.0f}")

    for pair_key in sorted(results.keys()):
        logger.debug("Language pair detected: %s", pair_key)

    if warnings:
        logger.info("Warnings/Errors collected: %s", len(warnings))
