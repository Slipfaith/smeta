...
class ProjectSetupWidget(QWidget):
    ...
    def _setup_ui(self, initial_volume: float):
        ...
        subtotal_row.setContentsMargins(0, 0, 0, 0)
        ...
        self.modifiers_button.setFixedWidth(32)
        self.modifiers_button.clicked.connect(self._show_modifiers_menu)
        subtotal_row.addWidget(self.modifiers_button)

        vbox.addLayout(subtotal_row)
    ...
    def _update_discount_label(self) -> None:
        ...
        discount_percent = self._discount_percent if self.group_box.isChecked() else 0.0
        markup_percent = self._markup_percent if self.group_box.isChecked() else 0.0
        ...
        prefix_amount = base_total if self.group_box.isChecked() else final_total
    ...
    def _show_modifiers_menu(self) -> None:
        ...
        def create_radio_action(text: str, with_spin: bool = False):
        ...
        def update_totals_label():
            discount_value = (
                float(discount_spin.value())
                if discount_spin and discount_radio.isChecked()
                else 0.0
            )
            markup_value = (
                float(markup_spin.value())
                if markup_spin and markup_radio.isChecked()
                else 0.0
            )
            preview_total = base_total - base_total * (discount_value / 100.0)
            preview_total += base_total * (markup_value / 100.0)
        ...
        menu.exec(self.modifiers_button.mapToGlobal(self.modifiers_button.rect().bottomLeft()))
