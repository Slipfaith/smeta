...
class AdditionalServiceTable(QWidget):
    ...
    def _update_discount_label(self) -> None:
        ...
        discount_amount = self._subtotal * (discount_percent / 100.0)
        markup_amount = self._subtotal * (markup_percent / 100.0)
        ...
        prefix = f"{tr('Промежуточная сумма', self.lang)}: {self._format_currency(self._subtotal)}"
        if parts:
